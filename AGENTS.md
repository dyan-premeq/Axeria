...
- 目标环境是 Unity `2022.3.62f3c1`、URP `14.0.12`。
...
- `_WorldSpaceDepth`、`_UsePlanetCenterUp` 与 `_PlanetCenter` 属于项目的行星空间约定。修改深度方向时，同时验证平面水体的 object-up 回退和球形世界的 radial-up 路径。
...
- 根据改动覆盖浅水/深水颜色、世界空间与相机空间深度、交界泡沫、Planet Center Up、透明排序和 GPU Instancing 中相关的路径。
